...
public class BookingInventoryImpl implements BookingInventory{
     ...
     * @return
     */
    public Booking retrieveBookingByCarReg(String vehicleRegReg){
        Booking booking = new Booking();
        ...
        try {
            String findQuery ="SELECT * FROM BOOKING b WHERE b.VEHICLE_ID = (SELECT v.VEHICLE_ID FROM VEHICLE v WHERE v.REGISTRATION '" + vehicleRegReg + "')";
            Statement statement = con.createStatement();
            ResultSet rs = statement.executeQuery(findQuery);
            while (rs.next()) {
                booking.setBookingId(rs.getInt(1));
